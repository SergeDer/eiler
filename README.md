...
В репозитории есть CLI-клиент `client.py` для отправки запросов к сервису.

```bash
python client.py "анна"
```

Параметры:

- обязательный позиционный аргумент — строка до 255 символов;
- `--url` — полный URL эндпойнта (по умолчанию `http://localhost:5000/palindromes`).
